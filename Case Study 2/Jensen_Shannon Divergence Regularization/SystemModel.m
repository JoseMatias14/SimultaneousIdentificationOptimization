function [alg,x_var,u_var,f_var,L,xk,yk,grad_yk,DxDu1,gradU1,DxDu2,gradU2] = SystemModel(xk_1,uk,fk,par,modelFlag)
...
%     a11 = par.a11; %0.7210 [-]
    ...
    %adjustable
    %Compressor Head
    c5 = MX.sym('c5'); %-0.9937 [s2/m6]
    c6 = MX.sym('c6'); %2.256 [s/m3]
    c7 = MX.sym('c7'); %1.888 [-]
...
%     c13 = SX.sym('c13'); %[-]
    ...
    c14 = MX.sym('c14'); %5 [kg2/m6]
    c15 = MX.sym('c15'); %7 [(m^{13/2} s)/kg^{5/2}]
    ...
    u_choke = MX.sym('u_choke'); % u_1 = 0.565 [%]
    %compressor speed (normalized)
    u_comp = MX.sym('u_comp'); % u_2 = 0.85[%]

    % Feed
    %mass flow gas from the reservoir
    m_res_gas = MX.sym('m_res_gas'); %0|1.4616 [kg/s] -- 1e2
    %mass flow liquid from the reservoir
    m_res_liq = MX.sym('m_res_liq'); %0.9|0.8997 [kg/s] -- 1e2
    %reservoir pressure
    P_res = MX.sym('P_res'); % 1.0 [Pa]  -- 1e7
    %reservoir temperature
    T_res = MX.sym('T_res'); % 3.5 [K]  -- 1e2 

    %% Variables
    %Reservoir
    %mass flow from the reservoir
    %volumetric flow from the reservoir
    q_res = MX.sym('q_res'); %[m3/s]

    %Choke
    %choke outlet pressure
    P_choke_out = MX.sym('P_choke_out'); % [Pa] -- 1e7

    %Compressor
    %inlet volumetric flowrate
    q_comp_in = MX.sym('q_comp_in'); % [m3/s]
    %temperature
    T_comp_out = MX.sym('T_comp_out'); % [K] -- 1e2
    %pressure
    P_comp_out = MX.sym('P_comp_out'); % [Pa] -- 1e7
    %compressor efficiency
    nu = MX.sym('nu'); %[-]
    %compressor head
    H = MX.sym('H'); % [m] -- 1e1
    %compressor power
    Pow = MX.sym('Pow'); %[W] -- 1e4

    %Pump
    %inlet volumetric flowrate
    q_pump_in = MX.sym('q_pump_in'); % [m3/s] -- 1e-1
    %compressor power
    Pop = MX.sym('Pop'); %[W] -- 1e3
...
%     alg = substitute(alg,c13,par.c13);
    ...
    % ===================================    
    %gradient
    gradU1 = MX.sym('s_1',length(xk_1),1);
    %calculating gradients
    DxDu1 = jacobian(alg,x_var)*gradU1 + jacobian(alg,u_var{1});
    %gradient
    gradU2 = MX.sym('s_2',length(xk_1),1);
    %calculating gradients
    DxDu2 = jacobian(alg,x_var)*gradU2 + jacobian(alg,u_var{2});
