...
%         area([200, 250],[1, 1],'FaceColor',[0.5,0.5,0.5],'FaceAlpha',0.2,'LineStyle','none')
        ...
        title('Manipulated variable (Greedy MAP Control)')
        ylabel('v_{ch} [%]','FontSize',10)
        xlabel('SS periods [-]','FontSize',10)
        xlim([1,tEnd])
        ylim([0.5,0.8])
...
%         area([200, 250],[1, 1],'FaceColor',[0.5,0.5,0.5],'FaceAlpha',0.2,'LineStyle','none')
        ...
        title('Manipulated variable (Dual Optimal-Information Control)')
        ylabel('v_{ch} [%]','FontSize',10)
        xlabel('SS periods [-]','FontSize',10)
        xlim([1,tEnd])
        ylim([0.5,0.8])
...
%     
    ...
    ylabel('Model','FontSize',10)
    xlabel('SS periods [-]')
    title('Model with highest probability (Greedy MAP Control)')
...
%     
    ...
    ylabel('Model','FontSize',10)
    xlabel('SS periods [-]')
    title('Model with highest probability (Dual Optimal-Information Control)')
...
subplot(3,1,2)    
    ...
    xlabel('SS periods [-]')
    title('Model probability (Greedy MAP Control)')
...
subplot(3,1,3)
    ...
    legend(leg3,'FontSize',6,'Location','best')
    xlabel('SS periods [-]')
    title('Model probability (Dual Optimal-Information Control)')
...
for ii = 1:length(OF_b)
    if ~isnan(OF_b(ii))
        pDif_1 = [pDif_1, (OF_g(ii) - OF_b(ii))]; % greedy
        pDif_2 = [pDif_2, (OF_d(ii) - OF_b(ii))]; % dual
 
    else
        % greedy
        if ~isnan(OF_g(ii))
            pDif_1 = [pDif_1, OF_g(ii)]; 
        else
            pDif_1 = [pDif_1, 0]; 
        end
...
subplot(3,1,1), 
    plot(1:g.tEnd + 1,pDif_1,'-bo','MarkerSize',5)
    hold on
    plot(1:d.tEnd + 1,pDif_2,'-rd','MarkerSize',5)
    %hold on 
    %a = area([blThres + 1 dThres + 1],[100 100],'FaceColor','r');
    % a.FaceAlpha = 0.2;
    
    grid on
    title('Objective function difference')
    ylabel('\phi diff. [%]','FontSize',10)
    xlabel('SS periods [-]','FontSize',10)
    xlim([1,tEnd])
    
    leg = {'ECM - baseline','JSDR - baseline'}; 
    legend(leg,'Location','best','FontSize',9)

if pr(1)
    print(f7,'Results_Profit','-dpdf')
end
if pr(2)
