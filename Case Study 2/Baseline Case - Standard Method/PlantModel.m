...
%       count = counter indicating the compressor states
%
% Outputs:
%       xk = steady-state system state = f(uk,fk)
%       xmk = steady-state system state with added noise
...
import casadi.*
    ...
    f5h = ((n11*qN^2 + n12*qN + n13)/(qN^2 + n14*qN + n15));
    f5d1 =(n21*qN^2 + n22*qN + n23);
    f5d2 =(n31 + n32*qN.^2./(n33 + qN.^2.*(n34 + qN.^2/n35)));
    ...
    %pump power [1e7 W]
    f10 = (Pop*1e3) - ((q_pump_in*1e-1)*((P_comp_out*1e7) - (P_choke_out*1e7)));
    ...
    % concatenate states, inputs and feed
