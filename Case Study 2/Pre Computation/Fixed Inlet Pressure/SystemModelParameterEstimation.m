function [pHatk,psik] = SystemModelParameterEstimation(xk_1,uk,fk,par,yp,gyp,modelFlag)
...
%     alg = substitute(alg,c13,par.c13);
    ...
    if modelFlag == 1
        p_var = vertcat(n11,n12,n13,n14,n15);
    elseif modelFlag == 2
        p_var = vertcat(n21,n22,n23);
    else %modelFlag == 3
        p_var = vertcat(n31,n32,n33,n34,n35);
    end
    ...
    grady_uk = -jacobian(alg,x_var)\jacobian(alg,u_var);
    
  
    J = sqrt((yp - par.H*x_var)'*par.Q1*(yp - par.H*x_var)) + sqrt((gyp(:,1) - par.H*grady_uk(:,1))'*par.Q2*(gyp(:,1) - par.H*grady_uk(:,1)) + (gyp(:,2) - par.H*grady_uk(:,2))'*par.Q3*(gyp(:,2) - par.H*grady_uk(:,2)));
...
%    options.ipopt.return_status = true;
    ...
    if modelFlag == 1
        %thetaStar = [0.582;-2.398;2.75;-3.969;4.303]
%         lbp = vertcat(0,-3,2,-4,4);
%         ubp = vertcat(1,-2,3.1,-3,5.1);
%         p0 = vertcat(0.1,-2.5,2.5,-3,4.4);
        lbp = vertcat(0,-2.398,  2,-3.969,4);
        ubp = vertcat(1,-2.398,3.1,-3.969,5.1);
        p0 =  vertcat(0.1,-2.398,2.5,-3.969,4.4);
    elseif modelFlag == 2
        %thetaStar = [-0.5;1.8;-0.85]
%         lbp = vertcat(-1.1,1,-1.1);
%         ubp = vertcat(0,2.1,0);
%         p0 = vertcat(-0.5,1.5,-1);
        lbp = vertcat(-0.5,1,-1.1);
        ubp = vertcat(-0.5,2.1,0);
        p0 =  vertcat(-0.5,1.5,-1);
    else %modelFlag == 3
        %thetaStar = [0.43;0.7;5;1;2]
%         lbp = vertcat(0,0,4.5,0.5,1.5);
%         ubp = vertcat(1.1,1.1,5.5,1.5,2.5);
%         p0 = vertcat(0.1,0.5,5,1,2);
        lbp = vertcat(  0,  0,5,1,2);
        ubp = vertcat(1.1,1.1,5,1,2);
        p0 =  vertcat(0.1,0.5,5,1,2);
    end
    
    lbw = [lbw;lbx;uk;fk;lbp];%one is a dummy variable - represent time
    ubw = [ubw;ubx;uk;fk;ubp];
    wk = [wk;xk_1;uk;fk;p0];
