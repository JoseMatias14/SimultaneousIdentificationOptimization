...
ma.Klam = 0.5*eye(ma.nInput);%gradient filter
...
while count <= tEnd
    ...
    if stopFlag == 0
       ...
       %3. Optimizing 
        [pi_k,ukArray,prob_k,pi_k_index,epsk,lambdak] = MAPGreedy(xk,uk,fk,par,H*xmk,gradPlantHat,epsk,lambdak,prob_k,H,ma);    
        ...
        % Plant true model
        plantModelArray = [plantModelArray, pModel];
        ...
        %optimal decision of the three models given the current modifiers
        uModelArray = [uModelArray, zeros(2,3)];
        %total modifier related to the three models
        probModelArray = [probModelArray, zeros(3,1)];
        % model-greedy controls that is implement at each period
        modelArrayU = [modelArrayU, zeros(1,1)];
        %4. For plotting
        % Optimization
...
%1. simulate plant (SS) and store plant data
[xk,xmk,gradPlant] = PlantModel(xk,uk,fk,parPlant,countPlant);
